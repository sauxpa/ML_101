...
class QDAClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self):
        self.pi = None
        self.mu_1 = None
        self.mu_0 = None
    ...
    def fit(self, X, y):
        ...
        self.cov_0 = (np.sum([np.matmul(xj.reshape(p, 1), xj.reshape(1, p)) for xj, yj in zip(X_centered_0, self.y_) if yj == 0], axis=0))/count_0
        self.cov_1 = (np.sum([np.matmul(xj.reshape(p, 1), xj.reshape(1, p)) for xj, yj in zip(X_centered_1, self.y_) if yj == 1], axis=0))/count_1
    ...
    def decision_function(self, X):
        ...
        """
        return self.pi * multivariate_normal.pdf(X, mean=self.mu_1, cov=self.cov_1) - (1-self.pi) * multivariate_normal.pdf(X, mean=self.mu_0, cov=self.cov_0)

    def predict(self, X):
        # Check is fit had been called
        check_is_fitted(self, ['pi', 'mu_1', 'mu_0', 'cov_1', 'cov_0'])
